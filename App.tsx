...
import { 
  ...
  History, Calendar as CalendarIcon, Settings as SettingsIcon
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleCompose = async (post: WPPost) => {
    ...
    // Fetch both captions and hashtags concurrently
    ...
    // Format brand hashtag from settings
    const brandName = settings.brandWebsite.split('.')[0].replace(/[^a-zA-Z0-9]/g, '');
    const brandTag = `#${brandName.charAt(0).toUpperCase() + brandName.slice(1)}`;
    
    // Combine tags into a clean string
    const hashtagString = [
      brandTag,
      ...tags.niche.slice(0, 3),
      ...tags.broad.slice(0, 2),
      ...tags.trending.slice(0, 2)
    ].map(t => t.startsWith('#') ? t : `#${t.replace(/\s+/g, '')}`).join(' ');

    // Merge hashtags into variations
    const mergedVars = vars ? {
      [Platform.FACEBOOK]: `${vars[Platform.FACEBOOK]}\n\n${hashtagString}`,
      [Platform.INSTAGRAM]: `${vars[Platform.INSTAGRAM]}\n\n${hashtagString}`,
      [Platform.TWITTER]: `${vars[Platform.TWITTER]}\n\n${hashtagString}`,
      [Platform.LINKEDIN]: `${vars[Platform.LINKEDIN]}\n\n${hashtagString}`,
    } : null;

    setVariations(mergedVars);
    setHashtags(tags);
    setIsGeneratingCaption(false);
    setIsGeneratingTags(false);
    
    if (mergedVars) {
      setPerformanceScore(await predictPerformance(post.title, mergedVars[selectedPlatform]));
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-8">
        <div className="bg-white p-12 rounded-[3.5rem] text-center max-w-lg shadow-2xl border border-slate-100">
          ...
          <p className="text-slate-500 mb-10 font-medium leading-relaxed">To activate premium AI assets and Veo video rendering, you must select a paid API key from your project.</p>
          <button onClick={async () => { const aistudio = (window as any).aistudio; if (aistudio) { await aistudio.openSelectKey(); setHasApiKey(true); } }} className="w-full bg-indigo-600 text-white py-5 rounded-2xl font-black text-lg hover:bg-indigo-700 transition-all flex items-center justify-center gap-3">Connect API Key <ArrowUpRight size={20} /></button>
        </div>
      </div>
    );
  }
      ...
      <div className="col-span-12 lg:col-span-8 space-y-8">
        <div className="bg-white/90 rounded-[3rem] border border-white shadow-2xl overflow-hidden relative glass-panel">
          ...
          <div className="p-10 relative">
            {(isGeneratingVideo || isGeneratingImage || isGeneratingCaption) && <div className="absolute inset-0 bg-white/95 backdrop-blur-xl flex flex-col items-center justify-center p-12 text-center gap-6 z-30 rounded-[3rem]"><Loader2 className="animate-spin text-indigo-600" size={64} /><h3 className="text-slate-900 font-black text-xl">{videoStatus || (isGeneratingCaption ? 'Architecting Smart Content...' : 'Connecting to Gemini Cloud...')}</h3></div>}
            {currentPost ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-6">
                  ...
                  <div className="p-6 bg-slate-50 border border-slate-200 rounded-[2.5rem] space-y-4">
                    <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-2"><Hash size={16} /> Asset Tags</p>
                    <div className="flex flex-wrap gap-2">
                       {/* Brand Hashtag specifically highlighted */}
                       <span className="px-3 py-1.5 bg-indigo-100 text-indigo-700 text-[9px] font-black rounded-full ring-2 ring-indigo-200">
                         #{settings.brandWebsite.split('.')[0].charAt(0).toUpperCase() + settings.brandWebsite.split('.')[0].slice(1)}
                       </span>
                       {hashtags?.niche.slice(0, 3).map(t => <span key={t} className="px-3 py-1.5 bg-slate-200 text-slate-600 text-[9px] font-black rounded-full">#{t.replace(/\s+/g, '')}</span>)}
                    </div>
                  </div>
    ...
    <div className="grid grid-cols-12 gap-10">
      <div className="col-span-12 lg:col-span-8 space-y-10">
        <section className="bg-white p-10 rounded-[3rem] shadow-2xl glass-panel space-y-8">
           <h3 className="text-xl font-black text-slate-900 border-b border-slate-100 pb-6 flex items-center gap-3"><Globe className="text-indigo-600" /> Infrastructure & Automation</h3>
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4 col-span-2">
                <p className="text-xs font-black text-slate-500 uppercase tracking-widest">Daily Post Limit</p>
      ...
      <div className="col-span-12 lg:col-span-4 space-y-10">
        <section className="bg-white p-8 rounded-[3rem] shadow-2xl glass-panel space-y-6">
           ...
           <div className="space-y-6 pt-4 border-t border-slate-100">
              <div className="space-y-2">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Brand Mark (Transparent PNG)</p>
                <input type="file" ref={fileInputRef} onChange={handleLogoUpload} className="hidden" accept="image/png" />
                <button onClick={() => fileInputRef.current?.click()} className="w-full py-6 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center gap-2">
                  {settings.logoUrl ? <img src={settings.logoUrl} className="h-10 object-contain" /> : <><Upload size={20} className="text-slate-300" /><span className="text-[9px] font-black text-slate-400 uppercase">Upload PNG</span></>}
                </button>
              </div>
              <div className="space-y-1.5">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Primary Accent</p>
                <div className="flex items-center gap-3">
                  <input type="color" value={settings.branding.primaryColor} onChange={(e) => setSettings({...settings, branding: {...settings.branding, primaryColor: e.target.value}})} className="w-10 h-10 rounded-lg cursor-pointer bg-transparent border-none" />
                  <span className="text-xs font-mono font-bold uppercase">{settings.branding.primaryColor}</span>
                </div>
              </div>
           </div>
        </section>
        ...
        <section className="bg-white p-8 rounded-[3rem] shadow-2xl glass-panel space-y-6">
           <h3 className="text-xl font-black text-slate-900 flex items-center gap-3"><Cpu className="text-indigo-600" /> AI Lab Config</h3>
           <div className="space-y-6 pt-4 border-t border-slate-100">
              ...
              <div className="space-y-4">
                 <p className="text-xs font-black text-slate-500 uppercase tracking-widest">Output Language</p>
    ...
    <Layout activeTab={activeTab} setActiveTab={setActiveTab}>
      ...
      {activeTab === 'account' && (
        <div className="max-w-2xl mx-auto py-20 text-center animate-in zoom-in-95">
           ...
           <button onClick={() => setCurrentUser(null)} className="w-full py-5 bg-rose-50 text-rose-600 rounded-[2rem] font-black text-lg hover:bg-rose-100 transition-all border border-rose-100 shadow-xl active:scale-95">Sign Out</button>
        </div>
      )}
    </Layout>
  );
